...
export default function CertificateRequest() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(currentStep)) {
      try {
        // Send form data to your local Node.js Express server
        ...
        if (response.ok) {
          // If the database successfully saves it, it returns the real tracking number
          const pickupDateCalc = calculatePickupDate();
          setReferenceNumber(data.referenceNumber); // Uses the tracking number generated by MongoDB
          setPickupDate(pickupDateCalc);
          setIsSubmitted(true);
        } else {
          // Handle server-side errors (e.g., rate limit triggered)
          alert(data.error || 'Something went wrong. Please try again.');
        }
      } catch (error) {
        console.error('Frontend Fetch Error:', error);
        alert('Cannot connect to the server. Make sure your backend is running on port 5000.');
      }
    }
  };
